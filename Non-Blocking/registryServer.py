    ...
    def registerReplica(self, request, context):
        ...
        for i in range(len(registry_server.replicas)):
            if ip==registry_server.replicas[i].ip and port==registry_server.replicas[i].port:
                return registry_pb2.registerReplicaResponse(status='False! Replica with same address already exist!')
        ...
        if registry_server.HasField('p_address')==False:
            ...
            return registry_pb2.registerReplicaResponse(status='True')

        else:
            return registry_pb2.registerReplicaResponse(status='True',paddress=registry_server.p_address)
