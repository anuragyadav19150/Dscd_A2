...
class Server(server_pb2_grpc.Server_serviceServicer):
    ...
    def WriteRequestToPrimary(self, request, context):
        ...
        for i in range(len(replica.backup_replicas)):
                ...
                response=stub.WriteRequestToBackup.future((server_pb2.BackupWriteRequest(name=request.name,uuid=request.uuid,content=request.content,timestamp=res['timestamp'])))
                response.add_done_callback(callback)


        print('Write done and write to backup called.')
    ...
    def Write(self, request, context):
        ...
        if not replica.HasField('p_address'):
            ...
            for i in range(len(replica.backup_replicas)):
                channel = grpc.insecure_channel('localhost:'+replica.backup_replicas[i].port)
                stub = server_pb2_grpc.Server_serviceStub(channel)
                response=stub.WriteRequestToBackup.future((server_pb2.BackupWriteRequest(name=request.name,uuid=request.uuid,content=request.content,timestamp=timestamp)))
                response.add_done_callback(callback)
    ...
    def Delete(self, request, context):
        ...
        if not replica.HasField('p_address'):
            ...
            for i in range(len(replica.backup_replicas)):
                channel = grpc.insecure_channel('localhost:'+replica.backup_replicas[i].port)
                stub = server_pb2_grpc.Server_serviceStub(channel)
                response=stub.DeleteRequestToBackup.future((server_pb2.BackupDeleteRequest(uuid=request.uuid,timestamp=result['timestamp'])))
                response.add_done_callback(callback)
